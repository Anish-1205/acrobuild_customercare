...
These are **read-only**, **company-scoped** APIs. All data returned belongs to a single company configured on the server (`app.csCompanyId`). The client does not pass `companyId` in URLs.
...
```bash
curl.exe -H "apiKey: YOUR_CS_API_KEY" http://localhost:9091/api/cs/company
```
...
```powershell
Invoke-RestMethod -Uri "http://localhost:8081/api/cs/company" -Headers @{ apiKey = "YOUR_CS_API_KEY" }
```
...
1. **Company contact** — `GET /api/cs/company` (phone, email, address for “contact us” answers)
2. **Project list** — `GET /api/cs/projects` (which projects exist)
3. **Project detail** — `GET /api/cs/projects/{projectId}` (single project metadata, including `location`)
4. **Amenities** — `GET /api/cs/projects/{projectId}/amenities`
5. **Wings** — `GET /api/cs/projects/{projectId}/wings`
6. **Typologies** — `GET /api/cs/projects/{projectId}/typologies` or `GET /api/cs/wings/{wingId}/typologies`
7. **Inventory** — `GET /api/cs/wings/{wingId}/inventory?availableOnly=true` (available units for a wing)
...
Returns non-deleted projects for the configured company.
...
```
GET /api/cs/projects/{projectId}/amenities
```
...
```
GET /api/cs/projects/{projectId}/wings
```
...
```
GET /api/cs/projects/{projectId}/typologies
```
...
```
GET /api/cs/wings/{wingId}/typologies
```
...
```
GET /api/cs/wings/{wingId}/inventory?availableOnly=true
```
...
| `404` | Company not configured/found, or project/wing ID not found for this company |
...
After receiving `apiKey` and `{restUrl}` from AcroBuild:

```bash
# 1. Company
curl.exe -H "apiKey: YOUR_CS_API_KEY" {restUrl}/api/cs/company

# 2. Projects
curl.exe -H "apiKey: YOUR_CS_API_KEY" {restUrl}/api/cs/projects

# 3. Wings (replace PROJECT_ID)
curl.exe -H "apiKey: YOUR_CS_API_KEY" {restUrl}/api/cs/projects/PROJECT_ID/wings

# 4. Typologies by project
curl.exe -H "apiKey: YOUR_CS_API_KEY" {restUrl}/api/cs/projects/PROJECT_ID/typologies

# 5. Inventory (replace WING_ID)
curl.exe -H "apiKey: YOUR_CS_API_KEY" "{restUrl}/api/cs/wings/WING_ID/inventory?availableOnly=true"
```
...
| `app.csCompanyId` | Company whose data all endpoints return |
...
# Api key
- api-key: a7f3c9e2b1d84f6a9e0c2b5d8f1a4e7